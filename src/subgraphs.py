...
class Grid(Mobject):
    def __init__(self, m, n, scale=0.2, node_constructor=Circle, infinite_rows=False, infinite_columns=False, color=PINK, toroidal=False, **kwargs):
        super().__init__()
                    ...
                    for vdir in vdirs:
                        ...
                        if min(vi, vj) >= 0 and vi < m and vj < n:
                            if toroidal and ((abs(vi - i)  == m-1 and m > 1) or (abs(vj - j) == n-1 and n > 1)):
                                if vj < j: continue
                                edge_object = CurvedEdge(start=to_3d(4*j*scale, 4*i*scale), end=to_3d(4*vj*scale, 4*vi*scale), color=BLUE, angle=TAU/8, buff=scale)
                            else:
    ...
    def ColorNode(self, i, j, color_number, label_text=None, label_color=BLACK):
        ...
        def color_node(node):
            ...
            node.set_fill(opacity=fill_opacity)
...
class Subgraphs(Slide):
    def construct(self):
        ...
        title = Tex(r"Finite subgraphs for lower bounds on $\chi_\rho(\mathbb{Z}^2)$", color=YELLOW).scale_to_fit_width(11)
        title.to_edge(UP)
        ...
        g_66 = Grid(6, 6, scale=0.25).scale(0.5).next_to(title, DOWN, buff=1.5*LARGE_BUFF)
